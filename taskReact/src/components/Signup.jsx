...
const Signup = () => {
    ...
    let { isAuthenticated , setIsAuthenticated, setLoading  } =
      useContext(Context);

    const submitHandler = async (e) => {
        console.log("http://localhost:4000/api/v1/users/create");
        setLoading(true);
        e.preventDefault();
  ...
  <div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
      ...
      <div class="w-full bg-white rounded-lg shadow dark:border md:mt-0 sm:max-w-md xl:p-0 dark:bg-gray-800 dark:border-gray-700">
          <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
              ...
              <form class="space-y-4 md:space-y-6" action={submitHandler}>

                  <div>
                      <label  for="fullname" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Full Name</label>
                      <input type="fullname"  onChange={(e) => setName(e.target.value)} name="confirm-password" id="confirm-password" placeholder="Full Name" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" required="" />
                  </div>
